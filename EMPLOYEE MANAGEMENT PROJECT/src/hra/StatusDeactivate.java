...
public class StatusDeactivate implements ActionListener{
    JFrame f;
    JTextField t;
    ...
    StatusDeactivate(){
        f=new JFrame("DEACTIVATE EMPLOYEE");
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                ...
                String str = "select EmployeeID,FirstName,Status,Role from project1.employee where EmployeeID ='"+t.getText()+"' ";
        ...
        if(ae.getSource()==b1){
        	if(role.equals("Hra")) {
        		JOptionPane.showMessageDialog(null,"Cannot Be Deactivated");
        	
        }else {
            try{
                DbConnection con = new DbConnection();
                String str = "UPDATE  project1.employee SET Status = 'Deactive' where EmployeeID = '"+t.getText()+"'";
                con.s.executeUpdate(str);
                JOptionPane.showMessageDialog(null,"Status Deactivated");
               
                f.setVisible(false);
                new HraLogin();

            }catch(Exception ex){
                JOptionPane.showMessageDialog(null,"Exception Occured While Deatcivate Record "+ex);
            }
        }
        }
        if(ae.getSource()==b2){
